...
module.exports.run = async (bot, message, args) => {
    ...
    let check  = message.member.roles.has('568105173591785472')||message.member.roles.has('545275416232067072')||message.member.roles.has("591292951905304652")||message.member.roles.has("598845743431680020")||message.member.hasPermission("ADMINISTRATOR");    if(!check) return;
    ...
    if(!checkadmin){
        ...
        if(!user.voiceChannel){
            ...
            const channel = message.guild.channels.find('id', `${message.member.voiceChannel.id}`);
