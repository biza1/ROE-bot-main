...
module.exports.run = async (__bot, message, args) => {
 ...
 User.find({}).sort({nhacnho:-1}).exec(async function(err, result) { 
   if(err) return message.channel.send("Đã xảy ra lỗi, vui lòng thử lại sau!");
    ...
    for(var i=0;i<10;i++){
      await punch.push(`Hạng ${i+1}: <@!${result[i].id}> ${result[i].nhacnho} tin nhắn\n`);
    if(9==i){
      let hit = new Discord.RichEmbed()
        .setDescription(`${punch.join("")}`)
        .setTitle("Top 10 người nói tục nhiều nhất")
        .setColor("#15edf1")
